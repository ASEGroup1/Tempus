...
CREATE TABLE IF NOT EXISTS `tempus`.`ModuleSessionStructure` (
  ...
  `MaxSessionSize`           INT NULL
  COMMENT 'In terms of number of students',
...
CREATE TABLE IF NOT EXISTS `tempus`.`ModuleTermNumber` (
  ...
  COMMENT 'Terms which the module is added to by default can contain multiple terms',
...
CREATE TABLE IF NOT EXISTS `tempus`.`DayTimingConstraint` (
  `DayTimingConstraintID`    INT NOT NULL AUTO_INCREMENT,
  `TimePeriodAvailabilityID` INT NULL,
  PRIMARY KEY (`DayTimingConstraintID`),
...
DROP TABLE `tempus`.`ModuleTimeTableSession`;
