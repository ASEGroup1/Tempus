...
object Dao {
  ...
  val timeTableHandler = new ResultSetHandler[Array[AnyRef]]() {
    @throws[SQLException]
    override def handle(rs: ResultSet): Array[AnyRef] = if (rs.next) Array(rs.getObject(2)) else Array()
  }

  val studentHandler = new ResultSetHandler[Array[AnyRef]]() {
    @throws[SQLException]
    override def handle(rs: ResultSet): Array[AnyRef] = if(rs.next) Array(Student(rs)) else Array()
  }

  val moduleHandler = new ResultSetHandler[Array[AnyRef]]() {
    @throws[SQLException]
    override def handle(rs: ResultSet): Array[AnyRef] = if(rs.next) Array(Module(rs)) else Array()
  }
  ...
  def delete(id: Int, tableName: String) =
  ...
  def getModule(id: Int) = run.query(conn, s"SELECT * FROM MODULE WHERE ID = $id", moduleHandler).head
